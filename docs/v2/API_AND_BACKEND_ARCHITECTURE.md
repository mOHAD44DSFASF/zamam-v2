...
| Attendance/Leave Services | manual attendance + evidence correction؛ leave request/ordered approval/balance ledger | Schedule/Holiday/People/Workload | attendance+correction atomic؛ leave+approval+balance+ledger atomic | no self approval؛ external HR read-only؛ final approval emits capacity event once |
| Reporting/Export | versioned fixed-formula KPI، reproducible measurements، scoped async CSV | Task/Workflow/Time/Attendance/Leave projections | immutable measurement by version+cutoff+hash؛ audited export job | performance/financial field policy؛ step-up؛ 10k row/30 field cap؛ CSV injection defense |
| Time Tracking Service | timer/manual entries/timesheet submit+decision/correction proposals | Task/Employment؛ time perms | entry/timesheet/correction + audit/outbox atomic | one timer؛ overlap/version؛ idempotent start؛ approved evidence immutable |
