...
describe('production readiness invariants',()=>{
  ...
  it('blocks Firebase predeploy while runtime composition or accountable launch evidence is missing',()=>{
    ...
    expect(result.stderr).not.toContain('WORKER_TRANSPORT_NOT_COMPOSED')
    expect(result.stderr).toContain('LAUNCH_AUTHORITY_NOT_APPROVED')
    expect(result.stderr).toContain('STAGING_ASSURANCE_EVIDENCE_MISSING')
  })
  ...
  it('wires a real worker event-processing pipeline instead of the disabled stub',()=>{
    const http=readFileSync(join(root,'services/workers/src/http.ts'),'utf8')
    expect(http).not.toContain('WORKER_TRANSPORT_NOT_CONFIGURED')
    expect(http).toContain('dispatchEvent')
  })
