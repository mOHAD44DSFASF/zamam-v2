...
describe('reports UI', () => {
  it('renders accessible RTL metrics and hides sensitive performance', async () => { const view = render(<ReportsScreen organizationId="org-1" client={client()} periodStart="2026-08-01" />); expect(await screen.findByRole('heading', { name: title })).toBeTruthy(); expect(screen.queryByText('\u0623\u062f\u0627\u0621 \u0627\u0644\u0641\u0631\u062f')).toBeNull(); expect((await axe(view.container)).violations).toEqual([]) })
  it('requests only server-advertised selected fields', async () => { const api = client(); render(<ReportsScreen organizationId="org-1" client={api} periodStart="2026-08-01" />); await screen.findByRole('heading', { name: title }); fireEvent.click(screen.getByRole('button', { name: '\u0637\u0644\u0628 \u062a\u0635\u062f\u064a\u0631 CSV' })); await waitFor(() => expect(api.requestExport).toHaveBeenCalledWith('org-1', expect.objectContaining({ requestedFields: ['metric','value'], scopeType: 'organization', scopeId: 'org-1' }))) })
})
