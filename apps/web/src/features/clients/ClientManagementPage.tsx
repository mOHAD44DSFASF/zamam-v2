      ...
      <div className="mx-auto grid max-w-7xl gap-0 px-5 py-7 lg:grid-cols-[320px_1fr]">
        ...
        <section className="border border-r-0 border-gray-200 bg-white p-6" aria-live="polite">
          {!selected ? <div className="grid min-h-72 place-items-center text-gray-500">اختر عميلاً لعرض التفاصيل.</div> : <>
            <div className="flex flex-wrap items-start justify-between gap-4 border-b pb-5"><div><h2 className="text-xl font-black">{selected.name}</h2><p className="mt-1 text-sm text-gray-500">{selected.industry ?? 'دون قطاع'} · {selected.activeProjectCount} مشروع نشط</p></div>{snapshot.capabilities.manageContacts && <button type="button" onClick={() => setDialog('contact')} className="inline-flex items-center gap-2 rounded-md border px-3 py-2 text-sm font-bold"><Plus className="size-4" aria-hidden="true" /> جهة اتصال</button>}</div>
