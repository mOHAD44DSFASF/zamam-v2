...
function TaskDetails({ task, canEdit, canTransition, onEdit, onTransition }: {
  ...
  return <>
    ...
    <nav aria-label="أقسام المهمة" className="flex gap-5 overflow-x-auto border-b py-4 text-sm font-bold"><span className="text-teal-800">نظرة عامة</span><span>المهام الفرعية</span><span>قائمة التحقق</span><a href={`/tasks/${task.id}/collaboration`} className="text-teal-800">التعليقات والنشاط</a><a href={`/files?resourceType=task&resourceId=${task.id}`} className="text-teal-800">الملفات</a></nav>
