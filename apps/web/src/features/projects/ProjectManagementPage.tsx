  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/45 p-4" role="dialog" aria-modal="true" aria-labelledby={titleId}>
      <form className="my-6 w-full max-w-2xl rounded-lg bg-white p-6" onSubmit={async (event) => {
        ...
        <div className="mt-6 flex justify-end gap-3"><button type="button" onClick={onClose} className="rounded-md border px-4 py-2 font-semibold">إلغاء</button><button type="submit" disabled={submitting || !snapshot.managers.length} className="rounded-md bg-teal-800 px-4 py-2 font-bold text-white disabled:opacity-50">إنشاء</button></div>
      </form>
    </div>
  )
}
    ...
    <main dir="rtl" className="min-h-screen bg-gray-50">
      ...
      <div className="mx-auto grid max-w-7xl px-5 py-7 lg:grid-cols-[330px_1fr]">
        ...
        <section className="border border-r-0 bg-white p-6">{!selected ? <div className="grid min-h-72 place-items-center text-gray-500">اختر مشروعًا.</div> : <>
          <div className="flex flex-wrap items-start justify-between gap-4 border-b pb-5"><div><div className="flex items-center gap-2"><h2 className="text-xl font-black">{selected.name}</h2><span className="rounded bg-gray-100 px-2 py-1 text-xs font-bold">{statusLabel[selected.status]}</span></div><p className="mt-1 text-sm text-gray-500">{selected.clientName ? `${selected.clientName} · ` : ''}<span dir="ltr">{selected.code}</span></p></div>{snapshot.capabilities.manage && selected.status === 'draft' && <button type="button" onClick={async () => { await client.transition(organizationId, selected.id, selected.version, 'planned'); await load() }} className="inline-flex items-center gap-2 rounded-md bg-teal-800 px-3 py-2 text-sm font-bold text-white">اعتماد الخطة</button>}{snapshot.capabilities.manage && selected.status === 'planned' && <button type="button" onClick={async () => { await client.transition(organizationId, selected.id, selected.version, 'active'); await load() }} className="inline-flex items-center gap-2 rounded-md bg-teal-800 px-3 py-2 text-sm font-bold text-white">تفعيل</button>}{snapshot.capabilities.manage && !['archived', 'cancelled'].includes(selected.status) && <button type="button" onClick={async () => { await client.setClientVisibility(organizationId, selected.id, selected.version, !selected.clientVisible); await load() }} className="inline-flex items-center gap-2 rounded-md border px-3 py-2 text-sm font-bold">{selected.clientVisible ? <EyeOff className="size-4" aria-hidden="true" /> : <Eye className="size-4" aria-hidden="true" />}{selected.clientVisible ? 'إخفاء عن العميل' : 'نشر للعميل'}</button>}</div>
