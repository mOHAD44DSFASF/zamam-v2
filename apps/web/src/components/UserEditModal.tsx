...
import { X, User, Shield, Edit3, ChevronDown, Check } from 'lucide-react';
  ...
  useEffect(() => {
    if (isOpen) {
      getDocs(collection(db, 'roles')).then(snapshot => {
        setRoles(snapshot.docs.map(d => ({ id: d.id, name: d.data().name })));
      }).catch(err => {
        console.error("Error loading roles in edit modal:", err);
      });
    }
  }, [isOpen]);
